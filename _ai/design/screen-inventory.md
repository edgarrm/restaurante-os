...
| 13 | Dashboard del día | `/dashboard` | Resumen de ventas, mesas activas y reservas del día para el admin. Reemplaza la ruta `dashboard` genérica del starter kit (antes en `routes/web.php`) — ahora es `role:admin` en `routes/tenant.php`, mismo nombre de ruta (ver `_ai/specs/dashboard-del-dia.spec.md`). | Ancla | Could | ⬜ | ✅ | ⬜ |
